...
T_SMOOTH = np.linspace(0, 1, 500)  # Número de pontos suavizados

def importar_circuito(pista):
# Teste importar Json e plotagem
    with open('circuitos.json', 'r') as f:
        data = json.load(f)
    return np.array(data[pista])

def coordenadas_pista(pista, width, length):
    pontos = importar_circuito(pista)
    x = pontos[:, 0]
    y = -pontos[:, 1]
    ...
    comp_atual = calc_comprimento(T_SMOOTH)

    escala = length / comp_atual
...
def criar_circuito(pista, width, length):
    coordenadas = coordenadas_pista(pista, width, length)
    x_center = coordenadas['x_center']
    y_center = coordenadas['y_center']
    x_inner = coordenadas['x_inner']
    y_inner = coordenadas['y_inner']
    x_outer = coordenadas['x_outer']
    y_outer = coordenadas['y_outer']
    ...
    plt.title(f"Circuito de {pista}")
    plt.grid(True)
    plt.gca().set_aspect('equal')
    plt.savefig(f"{pista}.png")

def melhor_tracado(pista, width, length):
    coordenadas = coordenadas_pista(pista, width, length)
    x_center = coordenadas['x_center']
    y_center = coordenadas['y_center']
    x_inner = coordenadas['x_inner']
    ...
    x_outer = coordenadas['x_outer']
    y_outer = coordenadas['y_outer']
    normals = coordenadas['normals']
    escala = coordenadas['escala']
    dx = coordenadas['dx']
    dy = coordenadas['dy']
    comp_atual = coordenadas['comp_atual']
    ...
    curvature = (dx*coordenadas['ddy'] - dy*coordenadas['ddx']) / (dx**2 + dy**2)**1.5
    ...
    plt.legend()
    plt.gca().set_aspect('equal')
    plt.title(f'Traçados {pista}')
    plt.savefig(f'tracados_{pista}.png')
    
    plt.figure(figsize=(20,12))
    plt.plot(x_center, y_center, 'k--', alpha=0.3, label=f'{pista} ({comp_atual/1000:.3f}km)')
    ...
    plt.legend()
    plt.gca().set_aspect('equal')
    plt.savefig(f'heatmap_{pista}.png')
    ...
    ani.save(f"simulacao_volta{pista}.mp4", writer="ffmpeg", fps=30)
    ...
    ani.save(f"animacao_carro_{pista}.mp4", writer="ffmpeg", fps=30)
    ...
    ani.save(f"animacao_carro_2_{pista}.mp4", writer="ffmpeg", fps=30)
    ...
    ani.save(f"animacao_carro_3_{pista}.mp4", writer="ffmpeg", fps=30)